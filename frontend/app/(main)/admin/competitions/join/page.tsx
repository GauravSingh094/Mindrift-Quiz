...
export default function JoinCompetitionPage() {
  ...
  useEffect(() => {
    ...
    const handleVisibilityChange = () => {
      const comp = validationState.competition;
      if (document.hidden && comp && !antiCheatState.isBlocked) {
        setAntiCheatState(prev => {
          const newCount = prev.tabSwitchCount + 1;
          const limit = comp.antiCheat.tabSwitchLimit;
          const newWarnings = [...prev.warnings];
          
          if (newCount >= limit) {
            // Block user
            const blockDuration = comp.antiCheat.cooldownPeriod * 60 * 1000;
            const blockEndTime = new Date(Date.now() + blockDuration);
            
            newWarnings.push(`BLOCKED: Too many tab switches (${newCount}/${limit}). Blocked for ${comp.antiCheat.cooldownPeriod} minutes.`);
            
            toast({
              variant: "destructive",
              title: "Competition Access Blocked",
              description: `You've been blocked for ${comp.antiCheat.cooldownPeriod} minutes due to excessive tab switching.`,
            });
